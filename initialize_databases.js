...
async function init() {
    ...
    try {
        const dbs = ['flights_bpp', 'international_flights_bpp', 'hotels_bpp'];
    ...
    // 2. Setup Flights BPP
    console.log('Setting up flights_bpp...');
    const flightsPool = new Pool({ ...config, database: 'flights_bpp' });
    try {
        const sql = `
        ...
        `;
        await executeSql(flightsPool, sql);
    } finally {
        await flightsPool.end();
    }

    // 3. Setup International Flights BPP
    console.log('Setting up international_flights_bpp...');
    const intlPool = new Pool({ ...config, database: 'international_flights_bpp' });
    try {
        const sql = `
            CREATE TABLE IF NOT EXISTS flights (
                id SERIAL PRIMARY KEY,
            ...
            ON CONFLICT (flight_code) DO NOTHING;
        `;
        await executeSql(intlPool, sql);

        // Load extra international flights from file if exists
        try {
            const extraSql = fs.readFileSync('international_flights_bpp.sql', 'utf8');
            // Very naive cleanup of comments and split by semicolon not robust for complex sql, 
            // but sufficient here if we remove \c command or just ignore errors
            // Actually, let's just create the extra table/inserts.
            // Because we are already connected, we just need to run the INSERTs.
            // The file has \c which we should ignore.
            const cleanSql = extraSql.replace(/\\c\s+\w+;/g, '');
            await executeSql(intlPool, cleanSql);
            console.log('Extra international flights loaded.');
        } catch (e) {
            console.log('Skipping extra international flights (file error or parse error).', e.message);
        }
    ...
    // 4. Setup Hotels BPP
    console.log('Setting up hotels_bpp...');
    const hotelsPool = new Pool({ ...config, database: 'hotels_bpp' });
    try {
        const sql = `
            ...
            VALUES
                ...
                ('OBERION-DEL-001', 'The Oberoi', 'Delhi', 'Dr Zakir Hussain Marg', '28.5921,77.2460', 5.0,
        ...
        `;
        await executeSql(hotelsPool, sql);
    } finally {
        await hotelsPool.end();
    }
