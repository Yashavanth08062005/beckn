    ...
    catalog.providers.forEach(provider => {
      if (provider.items) {
        provider.items.forEach(item => {
          // Filter logic:
          // If searching for BUS, only accept items with category_id === 'BUS'
          // If searching for FLIGHT, only accept items with category_id === 'FLIGHT'
          // If searching for HOTEL, only accept items with category_id === 'HOTEL' (or derived logic)

          const itemCategory = item.category_id; // "FLIGHT", "BUS", etc.
          ...
          if (mode === 'bus') {
            ...
            // Hotels might not have a category_id on the item directly in some implementations, 
            // or it is inferred. Logic in transformBecknItem suggests hotels are handled separately anyway.
            // We can check if provider categories include hospitality or inferred from intent context.
            // For now, let's assume filtering applies if category_id matches.
            // However, transformBecknItem handles 'hotel' mode.
            // Let's rely on transformBecknItem logic for filtering? No, better here.
            shouldInclude = true; // Hotels logic is usually distinct by intent category 'HOSPITALITY' which BAP separates
          }

          // Special case: If itemCategory is undefined, we might check provider category
          if (!itemCategory && mode === 'hotel') shouldInclude = true;
...
const createSearchIntent = (searchData) => {
  const intent = {
    category: {
      id: (searchData.transportMode === 'flight' || searchData.transportMode === 'bus') ? 'MOBILITY' : 'HOSPITALITY'
    }
  };

  if (searchData.transportMode === 'flight' || searchData.transportMode === 'bus') {
...
const transformBecknItem = (item, provider, transportMode) => {
  ...
  if (transportMode === 'flight' || transportMode === 'bus') {
    ...
    // For Buses, we might just get "City", so fallback to that or map it
    const departureCodeMatch = fromValue.match(/\(([^)]+)\)/);
    const arrivalCodeMatch = toValue.match(/\(([^)]+)\)/);

    let departureAirport = departureCodeMatch?.[1];
    let arrivalAirport = arrivalCodeMatch?.[1];

    if (transportMode === 'bus') {
      // For bus, if no code found in parens, use the full string or map it
      if (!departureAirport) departureAirport = fromValue.trim();
      if (!arrivalAirport) arrivalAirport = toValue.trim();
    } else {
      // Fallbacks for flights if needed
      departureAirport = departureAirport || item.fulfillment?.start?.location?.descriptor?.code;
      arrivalAirport = arrivalAirport || item.fulfillment?.end?.location?.descriptor?.code;
    }
    ...
    return {
      ...
      details: {
        airline: item.descriptor?.name || provider.descriptor?.name || (transportMode === 'bus' ? 'Bus Operator' : 'Airline'),
